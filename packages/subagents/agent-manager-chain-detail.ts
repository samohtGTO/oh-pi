...
export function renderChainDetail(state: ChainDetailState, chain: ChainConfig, width: number, theme: Theme): string[] {
	const lines: string[] = [];
	...
	const scrollInfo = formatScrollInfo(
		state.scrollOffset,
		Math.max(0, contentLines.length - (state.scrollOffset + CHAIN_DETAIL_VIEWPORT_HEIGHT)),
	);
