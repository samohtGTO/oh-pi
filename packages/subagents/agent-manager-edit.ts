...
export interface EditState {
	...
	isNew: boolean;
	fieldIndex: number;
	fieldMode: "text" | "model" | "thinking" | "skills" | null;
	fieldEditor: TextEditorState;
	...
	skillSelected: Set<string>;
	error?: string;
}
export interface EditInputResult {
	action?: "save" | "discard";
...
function formatTools(draft: AgentConfig): string {
	const tools = [...(draft.tools ?? []), ...(draft.mcpDirectTools ?? []).map((tool) => `mcp:${tool}`)];
	return tools.length > 0 ? tools.join(", ") : "";
}
function parseTools(value: string): { tools?: string[]; mcp?: string[] } {
	const items = value
		.split(",")
		.map((item) => item.trim())
...
function applyFieldValue(field: EditField, state: EditState, value: string): void {
	...
	switch (field) {
		...
		case "tools": {
			...
			draft.mcpDirectTools = parsed.mcp;
			break;
		}
		case "extensions": {
...
function openModelPicker(state: EditState, models: ModelInfo[]): void {
	state.fieldIndex = FIELD_ORDER.indexOf("model");
	state.fieldMode = "model";
...
function openSkillPicker(state: EditState, skills: SkillInfo[]): void {
	...
	state.fieldMode = "skills";
	state.skillSearchQuery = "";
	state.filteredSkills = [...skills];
	state.skillSelected = new Set(state.draft.skills ?? []);
	state.skillCursor = 0;
}
...
function renderModelPicker(state: EditState, width: number, theme: Theme): string[] {
	...
	if (list.length === 0) {
		...
		if (list.length > maxVisible) {
			startIdx = Math.max(0, state.modelCursor - Math.floor(maxVisible / 2));
...
function renderSkillPicker(state: EditState, width: number, theme: Theme): string[] {
	...
	if (list.length === 0) {
		...
		let startIdx = 0;
		if (list.length > SKILL_SELECTOR_HEIGHT) {
			startIdx = Math.max(0, state.skillCursor - Math.floor(SKILL_SELECTOR_HEIGHT / 2));
		...
		for (let i = startIdx; i < endIdx; i++) {
			...
			const isSelected = state.skillSelected.has(skill.name);
			const prefix = isCursor ? theme.fg("accent", "→ ") : "  ";
			const checkbox = isSelected ? theme.fg("success", "[x]") : "[ ]";
			const nameText = isCursor ? theme.fg("accent", skill.name) : skill.name;
			const sourceBadge = theme.fg("dim", ` [${skill.source}]`);
			const desc = skill.description ? theme.fg("dim", ` - ${truncateToWidth(skill.description, 25)}`) : "";
...
function renderPromptEditor(state: EditState, width: number, theme: Theme): string[] {
	...
	state.promptEditor.viewportOffset = ensureCursorVisible(
		cursorPos.line,
		PROMPT_VIEWPORT_HEIGHT,
...
export function handleEditInput(
	...
	if (screen === "edit") {
		...
		if (matchesKey(data, "down")) {
			state.fieldIndex = Math.min(FIELD_ORDER.length - 1, state.fieldIndex + 1);
			return;
		}
		const field = FIELD_ORDER[state.fieldIndex]!;
		if (data === "m") {
			openModelPicker(state, models);
			return { nextScreen: "edit-field" };
		}
		if (data === "t") {
			openThinkingPicker(state);
			return { nextScreen: "edit-field" };
		}
		if (data === "s") {
			openSkillPicker(state, skills);
			return { nextScreen: "edit-field" };
		}
	...
	if (screen === "edit-field") {
		...
		if (state.fieldMode === "thinking") {
			...
			if (matchesKey(data, "return")) {
				const selected = THINKING_LEVELS[state.thinkingCursor];
				state.draft.thinking = selected === "off" ? undefined : selected;
				state.fieldMode = null;
				return { nextScreen: "edit" };
			}
			if (matchesKey(data, "up")) {
				state.thinkingCursor = state.thinkingCursor === 0 ? THINKING_LEVELS.length - 1 : state.thinkingCursor - 1;
				return;
			}
			if (matchesKey(data, "down")) {
				state.thinkingCursor = state.thinkingCursor === THINKING_LEVELS.length - 1 ? 0 : state.thinkingCursor + 1;
				return;
			}
			return;
		}
		if (state.fieldMode === "skills") {
			...
			if (matchesKey(data, "return")) {
				...
				state.draft.skills = selected.length > 0 ? selected : undefined;
				state.fieldMode = null;
				return { nextScreen: "edit" };
			}
			if (data === " ") {
			...
			state.filteredSkills = query
				? skills.filter(
						(s) => s.name.toLowerCase().includes(query) || (s.description?.toLowerCase().includes(query) ?? false),
					)
				: [...skills];
		...
		if (matchesKey(data, "return")) {
			const field = FIELD_ORDER[state.fieldIndex]!;
			applyFieldValue(field, state, state.fieldEditor.buffer);
			state.fieldMode = null;
			return { nextScreen: "edit" };
		}
	...
	if (screen === "edit-prompt") {
		if (matchesKey(data, "escape") || matchesKey(data, "ctrl+c")) {
			state.draft.systemPrompt = state.promptEditor.buffer;
			return { nextScreen: "edit" };
		}
		const textWidth = Math.max(10, width - 4);
...
export function renderEdit(screen: EditScreen, state: EditState, width: number, theme: Theme): string[] {
	...
	for (let i = 0; i < FIELD_ORDER.length; i++) {
		...
		let valueText = renderFieldValue(field, state);
		if (field === "progress") {
			const toggle = state.draft.defaultProgress ? theme.fg("success", "[x]") : "[ ]";
			valueText = `${toggle} ${state.draft.defaultProgress ? "on" : "off"}`;
			lines.push(
				row(` ${prefix}${labelText} ${pad(truncateToWidth(valueText, valueWidth), valueWidth)}`, width, theme),
			);
			continue;
		}
		if (field === "interactive") {
			const toggle = state.draft.interactive ? theme.fg("success", "[x]") : "[ ]";
			valueText = `${toggle} ${state.draft.interactive ? "on" : "off"}`;
			lines.push(
				row(` ${prefix}${labelText} ${pad(truncateToWidth(valueText, valueWidth), valueWidth)}`, width, theme),
			);
			continue;
