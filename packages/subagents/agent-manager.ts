...
interface NameInputState {
	mode: "new-agent" | "clone-agent" | "clone-chain" | "new-chain";
...
function cloneChainConfig(config: ChainConfig): ChainConfig {
	return {
		...
		steps: config.steps.map((step) => ({
			...
			skills: Array.isArray(step.skills) ? [...step.skills] : step.skills,
		})),
	};
}
function slugTemplateName(name: string): string {
	return name
		.toLowerCase()
		.replaceAll(/[^a-z0-9]+/g, "-")
		.replaceAll(/^-+|-+$/g, "");
}
function nextSelectableIndex(items: TemplateItem[], current: number, direction: 1 | -1): number {
...
export class AgentManagerComponent implements Component {
	...
	private getChainEntry(id: string | null): ChainEntry | undefined {
		if (!id) {
			return undefined;
		}
		return this.chains.find((entry) => entry.id === id);
	}
	private listAgents(): ListAgent[] {
		const a = this.agents.map((entry) => ({
		...
		const c = this.chains.map((entry) => ({
			description: entry.config.description,
			id: entry.id,
			kind: "chain" as const,
			name: entry.config.name,
			source: entry.config.source,
			stepCount: entry.config.steps.length,
		}));
		return [...a, ...c];
	}
	...
	private removeAgentEntry(entry: AgentEntry): void {
		this.agents = this.agents.filter((e) => e.id !== entry.id);
		this.listState.selected = this.listState.selected.filter((id) => id !== entry.id);
	}
	private removeChainEntry(entry: ChainEntry): void {
		this.chains = this.chains.filter((e) => e.id !== entry.id);
	}
	...
	private enterChainDetail(entry: ChainEntry): void {
		this.currentChainId = entry.id;
		this.chainDetailState = { scrollOffset: 0 };
		this.screen = "chain-detail";
	}
	private enterEdit(entry: AgentEntry): void {
		this.currentAgentId = entry.id;
		this.editState = createEditState(entry.config, entry.isNew, this.models, this.skills);
		this.screen = "edit";
	}
	...
	private enterSavedChainLaunch(entry: ChainEntry): void {
		this.chainLaunchId = entry.id;
		this.chainAgentIds = [];
		this.parallelMode = false;
		this.taskBackScreen = "chain-detail";
		this.taskEditor = createEditorState();
		this.skipClarify = true;
		this.screen = "task-input";
	}
	private enterTemplateSelect(): void {
		this.templateCursor = TEMPLATE_ITEMS.findIndex((item) => item.type !== "separator");
		if (this.templateCursor < 0) {
			this.templateCursor = 0;
		}
		this.screen = "template-select";
	}

	private enterChainEdit(entry: ChainEntry): void {
		try {
			const content = fs.readFileSync(entry.config.filePath, "utf8");
			this.currentChainId = entry.id;
			this.chainEditState = { editor: createEditorState(content) };
			this.screen = "chain-edit";
		} catch (error) {
			this.statusMessage = {
				text: error instanceof Error ? error.message : "Failed to load chain file.",
				type: "error",
			};
			this.screen = "list";
		}
	...
	private enterNameInput(mode: NameInputState["mode"], sourceId?: string, template?: AgentTemplate): void {
		const allowProject = Boolean(this.agentData.projectDir);
		let initial = "";
		let scope: "user" | "project" = "user";
		if (mode === "clone-agent" && sourceId) {
			const entry = this.getAgentEntry(sourceId);
			if (entry) {
				initial = `${entry.config.name}-copy`;
				scope = entry.config.source === "project" ? "project" : "user";
			}
		}
		if (mode === "clone-chain" && sourceId) {
			const entry = this.getChainEntry(sourceId);
			if (entry) {
				initial = `${entry.config.name}-copy`;
				scope = entry.config.source === "project" ? "project" : "user";
			}
	...
	private saveEdit(): boolean {
		...
		if (!edit.draft.name || !edit.draft.description) {
			edit.error = "Name and description are required.";
			return false;
		}
		let { filePath } = entry.config;
		if (entry.isNew) {
			const dir = edit.draft.source === "project" ? this.agentData.projectDir : this.agentData.userDir;
			if (!dir) {
				edit.error = "Project agents directory not found.";
				return false;
			}
		...
		try {
			const toSave: AgentConfig = { ...edit.draft, filePath };
			fs.writeFileSync(filePath, serializeAgent(toSave), "utf8");
			entry.config = cloneConfig(toSave);
			entry.isNew = false;
			edit.error = undefined;
			return true;
		} catch (error) {
			edit.error = error instanceof Error ? error.message : "Failed to save agent.";
			return false;
		}
	}

	private saveChainEdit(): boolean {
		const state = this.chainEditState;
		const entry = this.getChainEntry(this.currentChainId);
		if (!state || !entry) {
			return false;
		}
		try {
			const parsed = parseChain(state.editor.buffer, entry.config.source, entry.config.filePath);
			fs.writeFileSync(entry.config.filePath, serializeChain(parsed), "utf8");
			entry.config = parsed;
			state.error = undefined;
			return true;
		} catch (error) {
			state.error = error instanceof Error ? error.message : "Failed to save chain.";
			return false;
		}
	}
	...
	private handleTemplateSelectInput(data: string): void {
		...
		if (matchesKey(data, "up")) {
			this.templateCursor = nextSelectableIndex(TEMPLATE_ITEMS, this.templateCursor, -1);
			this.tui.requestRender();
			return;
		}
	...
	private handleNameInput(data: string): void {
		...
		const innerW = this.overlayWidth - 2;
		const boxInnerWidth = Math.max(10, innerW - 4);
		const nextState = handleEditorInput(state.editor, data, boxInnerWidth);
		if (nextState) {
			state.editor = nextState;
			this.tui.requestRender();
			return;
		}
		if (matchesKey(data, "escape") || matchesKey(data, "ctrl+c")) {
		...
		const name = state.editor.buffer.trim();
		if (!name) {
		...
		if (state.mode === "clone-chain" && state.sourceId) {
			const sourceEntry = this.getChainEntry(state.sourceId);
			if (!sourceEntry) {
				this.screen = "list";
				this.tui.requestRender();
				return;
			...
			if (!dir) {
				state.error = "Project agents directory not found.";
				this.tui.requestRender();
				return;
			}
			const filePath = path.join(dir, `${name}.chain.md`);
			if (fs.existsSync(filePath)) {
				state.error = "A chain with that name already exists.";
				this.tui.requestRender();
				return;
			}
			try {
				...
				state.error = error instanceof Error ? error.message : "Failed to clone chain.";
				this.tui.requestRender();
				return;
			}
		}
		if (state.mode === "new-chain") {
			...
			const config: ChainConfig = {
				description: "Describe this chain",
				filePath,
				name,
				source: state.scope,
				steps: [{ agent: "agent-name", task: "{task}" }],
			};
			try {
				fs.mkdirSync(dir, { recursive: true });
				...
				state.error = error instanceof Error ? error.message : "Failed to create chain.";
			}
			this.tui.requestRender();
			return;
		}
		...
		if (state.mode === "clone-agent" && state.sourceId) {
			const sourceEntry = this.getAgentEntry(state.sourceId);
			if (!sourceEntry) {
				this.screen = "list";
				this.tui.requestRender();
				return;
	...
	private renderNameInput(w: number): string[] {
		...
		const lines: string[] = [];
		const title =
			state.mode === "new-agent"
				? "New Agent"
				: state.mode === "clone-agent"
					? "Clone Agent"
					: state.mode === "new-chain"
						? "New Chain"
						: "Clone Chain";
		lines.push(renderHeader(` ${title} `, w, this.theme));
		lines.push(row("", w, this.theme));
		lines.push(row(` ${this.theme.fg("dim", "Name:")}`, w, this.theme));
		const innerW = w - 2;
		const boxInnerWidth = Math.max(10, innerW - 4);
		...
		lines.push(row(` │${pad(editorLine, boxInnerWidth)}│`, w, this.theme));
		lines.push(row(` ${bottom}`, w, this.theme));
		if (state.mode === "new-agent" && state.template) {
			lines.push(row(` ${this.theme.fg("dim", "Template:")} ${state.template.name}`, w, this.theme));
		} else if (state.mode === "new-chain") {
			lines.push(row(` ${this.theme.fg("dim", "Creates a .chain.md configuration file")}`, w, this.theme));
		} else {
			lines.push(row("", w, this.theme));
		}
		if (state.allowProject) {
			const scopeLabel = state.scope === "user" ? "[user]" : "[proj]";
			lines.push(
				row(` ${this.theme.fg("dim", "Scope:")} ${scopeLabel}  ${this.theme.fg("dim", "[tab] toggle")}`, w, this.theme),
			);
		} else {
			lines.push(row("", w, this.theme));
		}
		if (state.error) {
			lines.push(row(` ${this.theme.fg("error", state.error)}`, w, this.theme));
		} else {
			lines.push(row("", w, this.theme));
		}
		lines.push(renderFooter(" [enter] continue  [esc] cancel ", w, this.theme));
		return lines;
	}

	private renderTemplateSelect(w: number): string[] {
		const lines: string[] = [];
		lines.push(renderHeader(" Select Template ", w, this.theme));
		lines.push(row("", w, this.theme));
		const innerW = w - 2;
		const viewport = 12;
		const start = Math.max(
			0,
			Math.min(this.templateCursor - Math.floor(viewport / 2), Math.max(0, TEMPLATE_ITEMS.length - viewport)),
		);
		const visible = TEMPLATE_ITEMS.slice(start, start + viewport);
		for (let i = 0; i < visible.length; i++) {
			...
			if (item.type === "separator") {
				...
				lines.push(
					row(
						` ${this.theme.fg("dim", label + "─".repeat(Math.max(0, innerW - 1 - visibleWidth(label))))}`,
						w,
						this.theme,
					),
				);
			} else {
				const isCursor = idx === this.templateCursor;
				const cursor = isCursor ? this.theme.fg("accent", "▸") : " ";
				const name = isCursor ? this.theme.fg("accent", item.name) : item.name;
				const desc = item.type === "agent" ? (item.config.description ?? "") : item.description;
				lines.push(
					row(
						` ${cursor} ${pad(name, 16)} ${this.theme.fg("dim", truncateToWidth(desc, Math.max(0, innerW - 24)))}`,
						w,
						this.theme,
					),
				);
		...
		const selected = TEMPLATE_ITEMS[this.templateCursor];
		const info = selected ? (selected.type === "separator" ? selected.label : selected.name) : "";
		lines.push(row(` ${this.theme.fg("dim", info)}`, w, this.theme));
		lines.push(renderFooter(" [enter] select  [esc] cancel  [↑↓] navigate ", w, this.theme));
		return lines;
	}

	private renderConfirmDelete(w: number): string[] {
		const agent = this.getAgentEntry(this.confirmDeleteId);
		const chain = this.getChainEntry(this.confirmDeleteId);
		const name = agent?.config.name ?? chain?.config.name ?? "";
		const filePath = agent?.config.filePath ?? chain?.config.filePath ?? "";
		const lines: string[] = [];
		lines.push(renderHeader(` Delete "${name}"? `, w, this.theme));
		lines.push(row("", w, this.theme));
		const label = "File: ";
		const maxPath = Math.max(0, w - 2 - label.length - 1);
		const trimmed = truncateToWidth(filePath, maxPath);
		lines.push(row(` ${label}${trimmed}`, w, this.theme));
		lines.push(row("", w, this.theme));
		lines.push(row(` ${this.theme.fg("warning", "This cannot be undone.")}`, w, this.theme));
		lines.push(row("", w, this.theme));
		lines.push(renderFooter(" [y] confirm  [n / esc] cancel ", w, this.theme));
		return lines;
	}
	...
	private renderChainEdit(w: number): string[] {
		...
		const lines: string[] = [];
		lines.push(renderHeader(` Edit Chain: ${entry.config.name} `, w, this.theme));
		lines.push(row("", w, this.theme));
		const innerW = w - 2;
		const boxInnerWidth = Math.max(10, innerW - 4);
		const top = `┌${"─".repeat(boxInnerWidth)}┐`;
		...
		const wrapped = wrapText(editorState.buffer, boxInnerWidth);
		const cursorPos = getCursorDisplayPos(editorState.cursor, wrapped.starts);
		editorState.viewportOffset = ensureCursorVisible(cursorPos.line, CHAIN_EDIT_VIEWPORT, editorState.viewportOffset);
		const editorLines = renderEditor(editorState, boxInnerWidth, CHAIN_EDIT_VIEWPORT);
		for (const line of editorLines) {
			lines.push(row(` │${pad(line, boxInnerWidth)}│`, w, this.theme));
		}
		lines.push(row(` ${bottom}`, w, this.theme));
		if (state.error) {
			lines.push(row(` ${this.theme.fg("error", state.error)}`, w, this.theme));
		...
		lines.push(renderFooter(" [ctrl+s] save  [esc] back ", w, this.theme));
		return lines;
	}

	handleInput(data: string): void {
		if (this.screen === "list" && this.statusMessage) {
			this.clearStatus();
		}
		if (this.screen.startsWith("edit") && this.editState?.error) {
			this.editState.error = undefined;
		}
		switch (this.screen) {
			case "list": {
			...
			case "template-select": {
				this.handleTemplateSelectInput(data);
				return;
			}
			case "detail": {
				const entry = this.getAgentEntry(this.currentAgentId);
				...
				if (action) {
					this.handleDetailAction(action, entry);
				}
				this.tui.requestRender();
				return;
			}
			case "chain-detail": {
				...
				const action = handleChainDetailInput(this.chainDetailState, data);
				if (action) {
					this.handleChainDetailAction(action, entry);
				}
				this.tui.requestRender();
				return;
			}
			case "parallel-builder": {
				...
				const agentOptions: AgentOption[] = this.agents.map((e) => ({
			...
			case "task-input": {
				if (matchesKey(data, "tab")) {
					this.skipClarify = !this.skipClarify;
					this.tui.requestRender();
					return;
				}
				const innerW = this.overlayWidth - 2;
				const boxInnerWidth = Math.max(10, innerW - 4);
				const nextState = handleEditorInput(this.taskEditor, data, boxInnerWidth);
				if (nextState) {
					this.taskEditor = nextState;
					this.tui.requestRender();
					return;
				}
				if (matchesKey(data, "escape") || matchesKey(data, "ctrl+c")) {
					this.screen = this.taskBackScreen;
					this.tui.requestRender();
					return;
				}
				if (matchesKey(data, "return")) {
					if (this.chainLaunchId) {
						...
						this.done({
							action: "launch-chain",
							chain: cloneChainConfig(chainEntry.config),
						...
						const tasks = this.parallelState.slots.map((slot) => ({
			...
			case "confirm-delete": {
				...
				if (data === "y" || data === "Y") {
					try {
						if (agent) {
							fs.unlinkSync(agent.config.filePath);
							this.removeAgentEntry(agent);
						} else if (chain) {
							fs.unlinkSync(chain.config.filePath);
							this.removeChainEntry(chain);
						}
			...
			case "chain-edit": {
				...
				if (
					...
					const { lines: wrapped, starts } = wrapText(this.chainEditState.editor.buffer, boxInnerWidth);
					const cursorPos = getCursorDisplayPos(this.chainEditState.editor.cursor, starts);
				...
				if (nextState) {
					this.chainEditState.editor = nextState;
					this.tui.requestRender();
					return;
				}
				if (matchesKey(data, "escape") || matchesKey(data, "ctrl+c")) {
					this.chainEditState = null;
					this.screen = "chain-detail";
					this.tui.requestRender();
					return;
				}
			...
			case "edit-prompt": {
				...
				const result = handleEditInput(
					this.screen as EditScreen,
					this.editState,
					data,
					this.overlayWidth,
					this.models,
	...
	private isBuiltin(id: string): boolean {
		const a = this.getAgentEntry(id);
		return a?.config.source === "builtin";
	}
	...
	private handleListAction(action: ListAction): void {
		switch (action.type) {
			case "open-detail": {
				...
				if (agent) {
					this.enterDetail(agent);
					return;
				}
				const chain = this.getChainEntry(action.id);
				if (chain) {
					this.enterChainDetail(chain);
				}
				return;
			}
			case "clone": {
			...
			case "new": {
				this.enterTemplateSelect();
				return;
			}
			case "delete": {
				if (this.isBuiltin(action.id)) {
					this.statusMessage = {
						text: "Builtin agents cannot be deleted. Clone to user scope to override.",
						type: "error",
					};
					return;
				}
				this.confirmDeleteId = action.id;
	...
	private handleDetailAction(action: DetailAction, entry: AgentEntry): void {
		...
		if (action.type === "edit") {
			if (entry.config.source === "builtin") {
				this.statusMessage = {
					text: "Builtin agents cannot be edited. Clone to user scope to override.",
					type: "error",
				};
				this.screen = "list";
				return;
	...
	private handleChainDetailAction(action: ChainDetailAction, entry: ChainEntry): void {
		...
		if (action.type === "launch") {
			this.enterSavedChainLaunch(entry);
			return;
		}
		if (action.type === "edit") {
			this.enterChainEdit(entry);
	...
	render(width: number): string[] {
		...
		switch (this.screen) {
			...
			case "template-select": {
				return this.renderTemplateSelect(w);
			}
			case "detail": {
				const entry = this.getAgentEntry(this.currentAgentId);
				if (!entry) {
			...
			case "chain-detail": {
				...
				return renderChainDetail(this.chainDetailState, entry.config, w, this.theme);
			}
			case "edit":
			case "edit-field":
			case "edit-prompt": {
				return this.editState ? renderEdit(this.screen as EditScreen, this.editState, w, this.theme) : [];
			}
			case "parallel-builder": {
				if (!this.parallelState) {
					return renderList(this.listState, this.listAgents(), w, this.theme, this.statusMessage);
			...
			case "task-input": {
				if (this.chainLaunchId) {
					const entry = this.getChainEntry(this.chainLaunchId);
					const title = entry ? `Chain: ${entry.config.name}` : "Chain";
					return renderTaskInput(title, this.taskEditor, this.skipClarify, w, this.theme);
				}
				if (this.parallelMode && this.parallelState) {
					return renderTaskInput(
						formatParallelTitle(this.parallelState.slots),
						this.taskEditor,
						this.skipClarify,
						w,
						this.theme,
			...
			case "confirm-delete": {
				return this.renderConfirmDelete(w);
			}
			case "name-input": {
				return this.renderNameInput(w);
			}
			case "chain-edit": {
				return this.renderChainEdit(w);
			}
		}
	}
