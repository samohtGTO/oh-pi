...
export type ParallelAction = { type: "proceed" } | { type: "back" };
...
function handleAdd(state: ParallelState, agents: AgentOption[], data: string): ParallelAction | undefined {
	...
	if (matchesKey(data, "up")) {
		state.addCursor = Math.max(0, state.addCursor - 1);
		return;
	}
	if (matchesKey(data, "down")) {
		state.addCursor = Math.min(Math.max(0, filtered.length - 1), state.addCursor + 1);
		return;
	}
...
export function renderParallel(state: ParallelState, agents: AgentOption[], width: number, theme: Theme): string[] {
	const lines: string[] = [];
	...
	if (state.mode === "browse") {
		...
		const slotStart = Math.max(
			0,
			Math.min(state.editIndex - Math.floor(slotLines / 2), state.slots.length - slotLines),
		);
