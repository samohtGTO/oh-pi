...
function renderFieldLine(label: string, value: string, width: number, theme: Theme): string {
	const labelWidth = 10;
...
function buildDetailLines(
	...
	if (agent.extraFields?.category) {
		lines.push(renderFieldLine("Category:", agent.extraFields.category, contentWidth, theme));
	}
	...
	const extensionsList =
		agent.extensions !== undefined ? (agent.extensions.length > 0 ? agent.extensions.join(", ") : "(none)") : "(all)";
...
export function handleDetailInput(state: DetailState, data: string): DetailAction | undefined {
	...
	if (matchesKey(data, "pageup") || matchesKey(data, "shift+up")) {
		state.scrollOffset -= DETAIL_VIEWPORT_HEIGHT;
		return;
	}
...
export function renderDetail(
	...
	const scrollInfo = formatScrollInfo(
		state.scrollOffset,
		Math.max(0, contentLines.length - (state.scrollOffset + DETAIL_VIEWPORT_HEIGHT)),
	);
