...
function availableNames(cwd: string, kind: "agent" | "chain"): string[] {
	...
	return [...new Set(items.map((x) => x.name))].toSorted((a, b) => a.localeCompare(b));
}
...
function unknownChainAgents(cwd: string, steps: ChainStepConfig[]): string[] {
	...
	return [...new Set(steps.map((s) => s.agent).filter((a) => !known.has(a)))].toSorted((a, b) => a.localeCompare(b));
}
...
function applyAgentConfig(target: AgentConfig, cfg: Record<string, unknown>): string | undefined {
	...
	if (hasKey(cfg, "skills")) {
		if (cfg.skills === false || cfg.skills === "") {
			...
			target.skills = skills.length > 0 ? skills : undefined;
		} else {
	...
	if (hasKey(cfg, "category")) {
		target.extraFields ??= {};
		if (cfg.category === false || cfg.category === "") {
			delete target.extraFields.category;
			if (Object.keys(target.extraFields).length === 0) {
				target.extraFields = undefined;
			}
		} else if (typeof cfg.category === "string") {
			target.extraFields.category = cfg.category.trim();
		} else {
			return "config.category must be a string or false when provided.";
		}
	}
	...
	if (hasKey(cfg, "reads")) {
		if (cfg.reads === false || cfg.reads === "") {
			...
			target.defaultReads = reads.length > 0 ? reads : undefined;
		} else {
...
export function formatAgentDetail(agent: AgentConfig): string {
	...
	if (agent.extraFields?.category) {
		lines.push(`Category: ${agent.extraFields.category}`);
	}
	if (tools.length > 0) {
	...
	if (agent.extraFields?.createdBy) {
		lines.push(`Created By: ${agent.extraFields.createdBy} (review system prompt before use)`);
	}
...
export function handleList(params: ManagementParams, ctx: ManagementContext): AgentToolResult<Details> {
	...
	const lines = [
		"Agents:",
		...(agents.length > 0 ? agents.map((a) => `- ${a.name} (${a.source}): ${a.description}`) : ["- (none)"]),
		"",
		"Chains:",
		...(chains.length > 0 ? chains.map((c) => `- ${c.name} (${c.source}): ${c.description}`) : ["- (none)"]),
	];
...
export function handleCreate(params: ManagementParams, ctx: ManagementContext): AgentToolResult<Details> {
	...
	if (isChain) {
		...
		fs.writeFileSync(targetPath, serializeChain(chain), "utf8");
	...
	fs.writeFileSync(targetPath, serializeAgent(agent), "utf8");
...
export function handleUpdate(params: ManagementParams, ctx: ManagementContext): AgentToolResult<Details> {
	...
	if (params.agent) {
		...
		fs.writeFileSync(updated.filePath, serializeAgent(updated), "utf8");
		if (updated.name !== oldName) {
	...
	fs.writeFileSync(updated.filePath, serializeChain(updated), "utf8");
	const headline =
		updated.name === oldName
