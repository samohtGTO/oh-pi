...
export function SidebarLayout({ children, currentView, onViewChange }: SidebarLayoutProps) {
		...
		<div className="flex min-h-screen">
			...
			>
				<div className="flex h-16 items-center justify-between border-b border-zinc-800 px-4">
					<div className="flex items-center gap-2">
						...
						{sidebarOpen && <span className="text-lg font-bold text-white">Analytics</span>}
					</div>
					<button
			...
			<main className={cn("flex-1 transition-all duration-300", sidebarOpen ? "lg:ml-64" : "lg:ml-16")}>
				...
				<header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-zinc-800 bg-zinc-950/80 backdrop-blur-xl px-4 sm:px-6">
					...
					<div className="flex items-center gap-2">
						<button className="rounded-lg p-2 text-zinc-400 hover:bg-zinc-800 hover:text-white" title="Export Data">
