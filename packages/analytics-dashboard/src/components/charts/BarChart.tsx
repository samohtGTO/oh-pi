...
export function BarChart({
	...
	// Limit data and sort
	const sortedData = data.slice(0, maxBars).toSorted((a, b) => a.value - b.value);
	...
	const CustomTooltip = ({
		...
		active?: boolean;
		payload?: { value: number; payload: BarDataItem }[];
		label?: string;
	}) => {
		if (active && payload && payload.length > 0) {
			...
			return (
				<div className="rounded-lg border border-zinc-800 bg-zinc-950 p-3 shadow-xl">
					...
					<p className="mt-1 text-lg font-bold text-primary-400">{item.displayValue ?? item.value}</p>
				</div>
			);
		}
		return null;
	...
	return (
		<div className={cn("w-full", className)}>
			...
			<ResponsiveContainer width="100%" height={height}>
				<RechartsBarChart data={sortedData} layout="vertical" margin={{ bottom: 0, left: 80, right: 30, top: 0 }}>
					<XAxis
						type="number"
